...
import { DEFAULT_BG_COLOR } from '@/features/avatar-picker/constants/avatar.ui.constants';

const CurrentAvatar = () => {
  const selectedBgColor = useAvatarStore((s) => s.selectedBgColor);
  const selectedAvatarId = useAvatarStore((s) => s.selectedAvatarId);

  const avatarId = getAvatarSrc({ avatar: selectedAvatarId });
  ...
  return (
    <div className="relative bg-white rounded-full p-2 shadow-2xl">
      ...
      >
        ...
        <AvatarImage loading="eager" fetchPriority="high" src={avatarId} alt="Current avatar" />
